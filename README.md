...
- Backend admin access is local to this app. Full admins can use the admin `Users` section to add backend users and assign one or more roles: `admin`, `user_admin`, `inventory_admin`, `pricing_admin`, `localline_pull`, `localline_push`, `square_pull`, `square_push`, `dropsite_admin`, `membership_admin`, and `member_admin`.
...
- Square Online price sync uses CSA Store as the source of truth, matches existing Square variations by reviewed name candidates, and pushes the CSA Store price for Square. Formula-priced products use Vendor's Retail Price directly without FFCSA factor, package weight/quantity, or customer markup; standard products use the local package price. Configure `SQUARE_ACCESS_TOKEN`, `SQUARE_ENVIRONMENT`, `SQUARE_LOCATION_ID`, optional `SQUARE_API_VERSION`, and `SQUARE_CURRENCY`.
