...
PORT=5177 NODE_ENV=production pm2 start apps/api/index.js --name store-api --cwd "$PWD" --update-env
wait_for_api

pm2 delete store >/dev/null 2>&1 || true
PORT=5176 NODE_ENV=production API_TARGET=http://127.0.0.1:5177 pm2 start server.js --name store --cwd "$PWD" --update-env
