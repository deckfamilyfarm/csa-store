...
router.post("/releases/:id/run-now", route(req => runProductSyncRelease(req.params.id, { user: req.admin, allowFuture: true })));
router.post("/releases/:id/retry", route(req => runProductSyncRelease(req.params.id, { user: req.admin, allowFuture: true })));
