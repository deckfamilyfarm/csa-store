...
- Products focuses on local catalog edits and has no sync button. Open Store → Product Sync from the menu to carry selected products and supported unsaved drafts into the sync workspace. Sync status, release history, and scheduling belong on Product Sync, not Products. Audit first, then explicitly approve platform actions. The old direct push endpoints remain compatible for existing clients.
